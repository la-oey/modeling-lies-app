                ...
                fluidRow(
                  column(width = 4, 
                         wellPanel(
                           ...
                           sliderInput('weight', HTML("lapse rate"), value = 0.05, min = 0, max = 1, step=0.05),
...
server <- function(input, output) {
  ...
  observe({
    if(!input$recursiveToM){
      freezeReactiveValue(input, "lambda")
      updateNumericInput(inputId = "lambda", value = 1, min = 1, max = 1)
    } else{
      freezeReactiveValue(input, "lambda")
      updateNumericInput(inputId = "lambda", value = 7, min = 1, max = 15)
    }
  })
